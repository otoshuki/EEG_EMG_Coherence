...
cmc_right_store = [];
cmc_left_store = [];
cmc_right_left_store = [];
cmc_left_right_store = [];
counter_our = 0;
...
for j = 44:44
    ...
    for i = 1:100
        ...
        %Class 3 - Right imagery
        eeg_right_left = eeg.imagery_right(50, j+1:k);
        emg_right_left = eeg.imagery_right(65, j+1:k);
        [cmc_right_left, f] = mscohere(eeg_right_left, emg_right_left, 512, 384, f, Fs);
        %Store data
        cmc_right_left_store = [cmc_right_left; cmc_right_left_store];
        
        %Class 4 - Left imagery
        eeg_left_right = eeg.imagery_left(13, j+1:k);
        emg_left_right = eeg.imagery_left(67, j+1:k);
        [cmc_left_right, f] = mscohere(eeg_left_right, emg_left_right, 512, 384, f, Fs);
        %Store data
        cmc_left_right_store = [cmc_left_right; cmc_left_right_store];
        j = k;
    end
    eeg.subject
end
...
data = [cmc_right_store(1:80,:); cmc_left_store(1:80,:); cmc_right_left_store(1:80,:); cmc_left_right_store(1:80,:)];
labels = [zeros(80,1); ones(80,1); 2*ones(80,1); 3*ones(80,1)];
out_cmc = [data, labels];
...
model = fitcecoc(out_kpls(:,1:10), labels);
...
new_data = [cmc_right_store; cmc_left_store];
